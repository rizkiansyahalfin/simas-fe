  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {col.sortable !== false && sortConfig?.key === col.accessorKey && (
                      sortConfig.direction === "asc" ? <ChevronUp className="h-4 w-4 text-emerald-600" /> : <ChevronDown className="h-4 w-4 text-emerald-600" />
                    )}
                  </div>
                </th>
              ))}
      ...
      <div className="p-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-500">
        <div>
          Menampilkan baris <span className="font-bold text-slate-700">{sortedData.length === 0 ? 0 : ((currentPage - 1) * itemsPerPage) + 1}</span> hingga <span className="font-bold text-slate-700">{Math.min(currentPage * itemsPerPage, sortedData.length)}</span> dari total <span className="font-bold text-slate-700">{sortedData.length}</span> data.
        </div>
        <div className="flex items-center gap-2">
          <Button 
            variant="outline" 
          ...
          <span className="font-bold text-slate-700 px-2">{currentPage} / {totalPages === 0 ? 1 : totalPages}</span>
          <Button 
            variant="outline" 
            size="icon"
            className="h-8 w-8 rounded-lg border-slate-200"
